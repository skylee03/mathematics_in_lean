...
theorem add_left_cancel {a b c : R} (h : a + b = a + c) : b = c := by
  rw [<- neg_add_cancel_left a b, h, <- add_assoc, add_comm (-a), add_right_neg, add_comm, add_zero]

theorem add_right_cancel {a b c : R} (h : a + b = c + b) : a = c := by
  nth_rw 1 [add_comm] at h
...
theorem mul_right_inv (a : G) : a * a⁻¹ = 1 := by
  ...
  rw [<- mul_assoc, mul_assoc (a⁻¹⁻¹), mul_left_inv, mul_assoc, one_mul, mul_left_inv]
